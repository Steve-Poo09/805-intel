...
const API_URL = "https://petroleum-crops-determine-lewis.trycloudflare.com";
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #0d0221 0%, #0a0e27 40%, #06141b 100%)", fontFamily: "'DM Sans', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, padding: "0 24px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ width: 72, height: 72, borderRadius: 20, margin: "0 auto 20px", background: "linear-gradient(135deg, rgba(255,107,53,0.3), rgba(247,37,133,0.2))", backdropFilter: "blur(20px)", border: "1px solid rgba(255,107,53,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, boxShadow: "0 0 40px rgba(255,107,53,0.2)" }}>📡</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.04)", backdropFilter: "blur(40px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: 32, boxShadow: "0 32px 64px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="text" value={u} onChange={e => setU(e.target.value)} placeholder="Enter username" onKeyDown={e => e.key === "Enter" && onLogin(u, p)} style={inputStyle}
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ position: "relative" }}>
              <input type={showP ? "text" : "password"} value={p} onChange={e => setP(e.target.value)} placeholder="Enter password" onKeyDown={e => e.key === "Enter" && onLogin(u, p)} style={{ ...inputStyle, paddingRight: 48 }}
                onFocus={e => { e.target.style.borderColor = "rgba(72,202,228,0.5)"; e.target.style.boxShadow = "0 0 0 3px rgba(72,202,228,0.1)"; }}
                onBlur={e => { e.target.style.borderColor = "rgba(255,255,255,0.1)"; e.target.style.boxShadow = "none"; }} />
...
function Dashboard({ onSignOut }) {
  ...
  // Fetch incidents from Pi backend
  useEffect(() => {
  ...
  const statusColor = status === "live" ? "#48cae4" : status === "error" ? "#ff6b35" : "#ffd166";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0d0221 0%, #0a0e27 40%, #06141b 100%)", fontFamily: "'DM Sans', sans-serif", color: "white", position: "relative" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: statusColor, boxShadow: `0 0 8px ${statusColor}`, animation: "livePulse 2s ease-in-out infinite" }} />
              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: 2, fontFamily: "'DM Mono', monospace" }}>{statusLabel}</span>
            </div>
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#48cae4", letterSpacing: 2 }}>
              {time.toLocaleTimeString("en-US", { hour12: false })}
            </span>
            <button onClick={onSignOut} style={{ background: "rgba(255,255,255,0.04)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "rgba(255,255,255,0.3)", fontSize: 10, letterSpacing: 2, padding: "6px 14px", cursor: "pointer", fontFamily: "'DM Mono', monospace", textTransform: "uppercase" }}>
              Sign Out
            </button>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px", position: "relative", zIndex: 1 }}>
        ...
        {/* Last updated */}
        {lastUpdate && (
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono', monospace", letterSpacing: 1, marginBottom: 12, textAlign: "right" }}>
            Last updated: {lastUpdate.toLocaleTimeString("en-US", { hour12: false })} · auto-refresh every 30s
          </div>
        )}
        ...
        {status === "connecting" && incidents.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono', monospace", fontSize: 12, letterSpacing: 3 }}>
            CONNECTING TO PI BACKEND...
          </div>
        ) : status === "error" && incidents.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 20px", color: "#ff6b35", fontFamily: "'DM Mono', monospace", fontSize: 12, letterSpacing: 3 }}>
            ⚠ CANNOT REACH PI BACKEND — CHECK TUNNEL
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,0.15)", fontFamily: "'DM Mono', monospace", fontSize: 12, letterSpacing: 3 }}>
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between" }}>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.15)", fontFamily: "'DM Mono', monospace", letterSpacing: 1 }}>SCOUTLABS / 805 INTEL v2.0 — SLO + SB COUNTY</span>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.15)", fontFamily: "'DM Mono', monospace", letterSpacing: 1 }}>{API_URL}</span>
        </div>
      </div>
    </div>
  );
